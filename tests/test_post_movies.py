...
class TestPostMovies:
    ...
    def test_add_movie_positive(self, super_admin_api_manager: ApiManager, movie_data):
    ...
    def test_add_movie_negative_invalid_data(self, super_admin_api_manager: ApiManager):
        ...
        response = super_admin_api_manager.movies_api.post_movie(data=invalid_data, expected_status=[400, 422])
        assert response is not None, "Response должен существовать"

    def test_add_movie_negative_missing_required_fields(self, super_admin_api_manager: ApiManager):
        """Негативный тест создания фильма без обязательных полей."""
        incomplete_data = {
            "name": "Test Movie"
            # Отсутствуют обязательные поля
        }
        response = super_admin_api_manager.movies_api.post_movie(data=incomplete_data, expected_status=[400, 422])
        assert response is not None, "Response должен существовать"