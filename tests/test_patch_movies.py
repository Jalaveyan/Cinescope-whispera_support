...
class TestPatchMovies:
    ...
    def test_patch_movie_positive(self, super_admin_api_manager: ApiManager, created_movie, movie_data):
        ...
        response = super_admin_api_manager.movies_api.patch_movie(created_movie, data=updated_data)
    ...
    def test_patch_movie_negative_not_found(self, super_admin_api_manager: ApiManager, movie_data):
        """Негативный тест обновления несуществующего фильма."""
        non_existent_id = 999999
        response = super_admin_api_manager.movies_api.patch_movie(
            non_existent_id, data=movie_data, expected_status=404
        )
        assert response is not None, "Response должен существовать"

    def test_patch_movie_negative_invalid_data(self, super_admin_api_manager: ApiManager, created_movie):
